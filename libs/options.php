 ...
 * @editor		Ilham Wahyudin <dev@studioinspirasi.com>
...
<?php	}
		...
		switch ( $field['type'] ) {
			...
			case 'media':
				...
				printf(
					...
					esc_attr($field['id']),                                                         // id
					esc_attr("{$page_key}[{$field['id']}]"),                                        // name
					!empty($field['placeholder']) ? 'placeholder="' . esc_attr($field['placeholder']) . '"' : '', // placeholder
					esc_attr($field['title_attr']),                                                 // title
					esc_attr($field['value']),                                                      // value
					!empty($attributes) ? implode(' ', array_map('esc_attr', $attributes)) : '',                           // additional attributes
					'<input class="button rational-media-upload" type="button" value="Upload">',                                                                  // upload button
					!empty($field['text']) ? '<p class="help">' . esc_html($field['text']) . '</p>' : '' // text
				);
				break;
			...
			case 'select':
				if (!empty($field['attributes']) && isset($field['attributes']['multiple']) && $field['attributes']['multiple']) {
					$field_tag_name = esc_attr("{$page_key}[{$field['id']}][]");
			...
			case 'textarea':
				printf(
					...
					esc_attr("{$page_key}[{$field['id']}]"), // name
					!empty($field['placeholder']) ? 'placeholder="' . esc_attr($field['placeholder']) . '"' : '', // placeholder
					!empty($field['rows']) ? 'rows="' . esc_attr($field['rows']) . '"' : '', // rows
					!empty($field['cols']) ? 'cols="' . esc_attr($field['cols']) . '"' : '', // cols
					!empty($field['wrap']) ? 'wrap="' . esc_attr($field['wrap']) . '"' : '', // wrap
					esc_attr($field['title_attr']), // title
			...
			case 'wp_editor':
				$field['textarea_name'] = esc_attr("{$page_key}[{$field['id']}]");
				wp_editor(isset($field['value']) ? $field['value'] : '', $field['id'], array(
					'textarea_name' => $field['textarea_name'],
				));
				echo !empty($field['text']) ? '<p class="help">' . esc_html($field['text']) . '</p>' : '';
				break;
			default:
				printf(
	...
	protected function build_settings_section( $page_key, $section_key ) {
		...
		if ( !empty( $section['include'] ) ) {
			include esc_html( $section['include'] );
		}
	}
	...
	public function submit_error( $error ) {
		$error = sprintf(
			...
			is_array( $error ) || is_object( $error ) ? '<pre>' . esc_html( print_r( $error, true ) ) . '</pre>' : esc_html( $error )
		);
